...
async function main() {
  ...
  const questions = JSON.parse(
    fs.readFileSync(path.join(root, "lib/question-library/ml-seed.json"), "utf8"),
  )

  if (!Array.isArray(questions) || questions.length !== 100) {
    console.error(`Expected 100 questions in ml-seed.json, got ${questions?.length}`)
    process.exit(1)
  }
  ...
  console.log("ML library seeded successfully via Supabase API.")
  console.log(`ML questions in database: ${count}`)
}
